...
@SuppressLint("NewApi")
public class MainCarteraClientes extends Activity {
	String codUser,nombreUsuario,tipoUsuario,scriptSQL,selectClientes,txtCliente,codigo,nombre,tipocli;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_main_cartera_clientes);
		...
		lvClientes = (ListView)findViewById(R.id.lvCarteraClientes);
		...
		EClientes = (EditText)findViewById(R.id.txtBusquedaClientesCartera);
		...
		try{
			...
			dialogo.miDialogoToastLargo(getApplicationContext(), "Error al conectarse a la BD: Clase MainCarteraClientes --> " + e.getMessage());
			return;
		}
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				if(RBNombre.isChecked())
				{
					selectClientes = "SELECT codigo,nombre,negocio,representa,tipo_identificacion,numero_identificacion,direccion," +
									 ...
									 "usuario_elimina,estado,cltenuevo,descargado,clavefe FROM cliente where nombre like '%"+txtCliente+"%' and " +
									 "(usuario = '"+codUser+"' or vendedor_aux='"+codUser+"')";
				}
				else
				{
					selectClientes = "SELECT codigo,nombre,negocio,representa,tipo_identificacion,numero_identificacion,direccion," +
							 		 ...
							 		 "dias_credito,usuario,forma_pago,iva,con_final,clase,observacion,tipo_negocio,ejex,ejey," +
							 		 "vendedor_aux,fecha_modifica,hora_modifica,usuario_modifica,fecha_elimina,hora_elimina," +
							 		 "usuario_elimina,estado,cltenuevo,descargado,clavefe FROM cliente where codigo like '%"+txtCliente+"%' and " +
							 		 "(usuario = '"+codUser+"' or vendedor_aux='"+codUser+"')";
				}
		...
		lvClientes.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			{
				codigo = cliAdapter.getItem(i).getCodigo();
				nombre = cliAdapter.getItem(i).getNombre();
				tipocli = cliAdapter.getItem(i).getTipoCliente();
						
				intent = new Intent(getApplicationContext(),MainVerCartera.class);
		...
		try{
			if(db!=null)
			{
				...
				try{
					...
					dialogo.miDialogoToastLargo(getApplicationContext(), "Error al ejecutar el script de pedido: Clase MainCarteraClientes --> " + e.getMessage());
					return;
				}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		getMenuInflater().inflate(R.menu.main_cartera_clientes, menu);
		return true;
	}
